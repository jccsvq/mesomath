...
options:
  ...
  -t {L,Lh,S,V,C,W,B,SysG,SysS,SysK}, --type {L,Lh,S,V,C,W,B,SysG,SysS,SysK}
...
* Metrological table type (options `-t` or `--type`)
    ...
    * SysK:   System SKL to count years in Sumerian King List
...
Will print the coefficients of the units expressed in the corresponding system C, S or G making the output more closely mimic the way the measurements were actually inscribed on the clay tablets, but it complicates things for the modern reader:
...
You can combine the `-F --fractions` options with `-a --academic` to obtain listings using the academic names of the units:
    ...
    $ metrotable -x 1 -aF1 -f0 -v$ metrotable -x 1 -aF1 -f0 -v
...
$ metrotable -t L -m '10 susi' -M '2 kus,12 kus,5 ninda' -i '5 susi,1 kus,6 kus' -vc
